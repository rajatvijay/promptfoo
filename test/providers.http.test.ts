...
import { executeExternalScript } from '../src/util/externalScript';
...
jest.mock('../src/util/externalScript', () => ({
  executeExternalScript: jest.fn(),
}));
...
describe('HttpProvider', () => {
  ...
  describe('HttpProvider with external scripts', () => {
    beforeEach(() => {
      jest.clearAllMocks();
    });

    it('should handle external JavaScript response parser', async () => {
      const mockExternalParser = jest.fn((json, text) => ({ parsed: json }));
      jest.mocked(executeExternalScript).mockResolvedValue(mockExternalParser);

      const provider = new HttpProvider(mockUrl, {
        config: {
          responseParser: 'file://path/to/external/parser.js',
        },
      });
      ...
      expect(executeExternalScript).toHaveBeenCalledWith(
        'file://path/to/external/parser.js',
        mockResponse.data,
        mockResponse.data,
        'get_response_parser'
      );
      expect(mockExternalParser).toHaveBeenCalledWith(mockResponse.data, JSON.stringify(mockResponse.data));
      expect(result.output).toEqual({ parsed: { result: 'success' } });
    });

    it('should handle external Python response parser', async () => {
      const mockPythonResult = { parsed: 'python result' };
      jest.mocked(executeExternalScript).mockResolvedValue(mockPythonResult);

      const provider = new HttpProvider(mockUrl, {
        config: {
          responseParser: 'file://path/to/external/parser.py',
        },
      });
      ...
      expect(executeExternalScript).toHaveBeenCalledWith(
        'file://path/to/external/parser.py',
        mockResponse.data,
        mockResponse.data,
        'get_response_parser'
      );
      expect(result.output).toEqual(mockPythonResult);
    });

    it('should handle errors from external scripts', async () => {
      jest.mocked(executeExternalScript).mockRejectedValue(new Error('External script error'));

      const provider = new HttpProvider(mockUrl, {
        config: {
          responseParser: 'file://path/to/external/error_parser.js',
        },
      });
      ...
      await expect(provider.callApi('test prompt')).rejects.toThrow('External script error');
    });

    it('should handle non-function results from external scripts', async () => {
      jest.mocked(executeExternalScript).mockResolvedValue({ notAFunction: true });

      const provider = new HttpProvider(mockUrl, {
        config: {
          responseParser: 'file://path/to/external/invalid_parser.js',
        },
      });
